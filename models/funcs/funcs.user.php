<?php
/* Get the users ID from their unique token
 * Pass the users 20 char token, recieve their user ID (integer)
 */
function getIDfromToken($user_token){
	$Data = new Data;
	$user_id = $Data->getData("Member_Users", "User_ID", array("User_Token"=>$user_token));
	if($user_id){
